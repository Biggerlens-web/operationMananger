<template>
    <el-dialog :model-value="dialogVisible" width="1300" title="其他尺寸模板" :before-close="handleClose">
                ...
                <template #item="{ element, index }">
                    <div class="template-card" :class="{ 'dragging': isDragging }">
                        ...
                        <div class="card-image">
                            <img v-lazy="element.coverUrl" :alt="'模板'" />
                        </div>

                        <div class="card-actions">
                            <el-button size="small" type="primary" @click="editTemplate(element)">编辑</el-button>
                            <el-button size="small" type="danger" @click="editTemplateFore(element)">模板前景</el-button>
                        </div>
                    </div>
                </template>
            </draggable>
        ...
        <template #footer>
            <el-button size="small" type="primary" @click="addChildTemplate">新增子模版</el-button>
...
<script lang="ts" setup>
    ...
    const getChildList = async () => {
        try {
            const params = {
                ...
                parentId: props.isAddChild
            }
            console.log('参数', params);
            const enData = desEncrypt(JSON.stringify(params))
            showLoading.value = true
            const res = await service.get('/templateUpDetail/childlist', {
                params: {
                    enData
                }
            })
            console.log('获取子列表', res);
    ...
    const editTemplate = (item: any) => {
        ...
        console.log('编辑子模版', item);
        emit('editChildTemplate', item)
    }

    //编辑子模版前景
    const editTemplateFore = (item: any) => {
        console.log('编辑子模版前景', item);
    }
