...
<script setup lang="ts">
    ...
    const { showPagestion, regionList, operationClass, templateFliterParams, defaultAppNo, showLoading } = storeToRefs(counterStore)
    ...
    const moveIndex = async (item: any) => {
        ...
        try {
            ...
            const res = await service.post('/templateUp/move', {
                enData
            })
            console.log('移动', res);
    ...
    const getClothList = async () => {
        ...
        try {
            ...
            console.log('搜索模板参数', params);
            const enData = desEncrypt(JSON.stringify(params))
            showLoading.value = true
            const res: any = await service.post('/templateUp/list', {
                enData
            })

            console.log('获取模板分类列表', res);
    ...
    const searching = () => {
        ...
        console.log('templateFliterParams', templateFliterParams.value);
        getClothList();
    }
    ...
    onMounted(async () => {
        ...
        if (Object.keys(templateFliterParams.value).length > 0) {
            searchParams.value = {
                ...templateFliterParams.value
            }
